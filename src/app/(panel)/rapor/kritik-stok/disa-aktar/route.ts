...
/**
 * Aynı `kritikStokIdleriGetir` kaynağından besleniyor — bkz. `../page.tsx`
 * ve `/stok/minimum/disa-aktar/route.ts` (aynı mantığın bilinçli tekrarı).
 */
export async function GET() {
  ...
  await logKaydet({
    ...
    tablo: "stoklar",
    aciklama: `Kritik Stok raporu dışa aktarıldı (${kayitlar.length} kayıt)`,
  })

  const bugun = new Date().toISOString().slice(0, 10)
  return csvYaniti(icerik, `kritik-stok-rapor-${bugun}.csv`)
}
