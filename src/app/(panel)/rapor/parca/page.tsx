...
export const metadata: Metadata = { title: "Yapılan Parçalar" }
export const dynamic = "force-dynamic"

/**
 * YAPILAN PARÇALAR (ADIM 10.2)
 *
 * Yapılan İşçilikler'in aynası — stok kartı bazında gruplu döküm, teslim
 * edilen kartların parça kalemleri. Bkz. rapor/veri.ts açıklaması.
 */
export default async function YapilanParcalar({
  searchParams,
}: {
  ...
  return (
    <div className="flex flex-col">
      <div className="sayfa-basligi">
        <div>
          <h1 className="text-[1.0625rem] font-semibold tracking-tight">Yapılan Parçalar</h1>
          <p className="text-[0.8125rem] text-muted-foreground">
            Stok kartı bazında gruplu döküm (teslim edilen kartlar)
          </p>
        </div>
        <div className="flex items-center gap-2">
          <YazdirDugmesi />
          <DisaAktarDugmesi yol={`/rapor/parca/disa-aktar${raporSorgusu(filtreler)}`} />
        </div>
      </div>

      <div className="yazdirma-disi">
        <TarihAraligiFiltre yol="/rapor/parca" bas={filtreler.bas} bit={filtreler.bit} />
      </div>
