...
const TUR_ADI: Record<string, string> = {
  ...
  PERAKENDE: "Hızlı Satış",
  IADE_SATIS: "İade Faturası",
}
...
/** SATIŞ EVRAKLARI CSV. Ekrandaki filtrenin (ara / durum / tarih) AYNISINI kullanır. */
export async function GET(istek: NextRequest) {
  ...
  await logKaydet({
    ...
    tablo: "evraklar",
    aciklama: `Satış evrakları listesi dışa aktarıldı (${evraklar.length} kayıt)`,
    yeniDeger: f,
  })

  const bugun = new Date().toISOString().slice(0, 10)
  return csvYaniti(icerik, `satis-evraklari-${bugun}.csv`)
}
