...
import { satisEvraklariGetir } from "./veri"
...
export const metadata: Metadata = { title: "Satış Evrakları" }
export const dynamic = "force-dynamic"

type Aramalar = { q?: string; durum?: string; bas?: string; bit?: string }

/** Satış fatura listesi — Depo Transferi'ndeki fiş listesi deseninin aynısı. */
export default async function SatisEvraklari({
  searchParams,
}: {
  ...
  const evraklar = await satisEvraklariGetir(f)
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[1rem] font-semibold">Satış Evrakları</h1>
        ...
        <div className="flex items-center gap-2">
          <DisaAktarDugmesi
            yol={`/evrak/satis/disa-aktar${disaAktarSorgu ? `?${disaAktarSorgu}` : ""}`}
          />
          <Button size="sm" asChild>
            <Link href="/evrak/satis/yeni">
