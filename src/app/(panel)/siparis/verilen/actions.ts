...
/** Verilen sipariş kartını kaydeder (yeni veya taslakken güncelleme). */
export async function siparisKaydet(
  ...
  try {
    kayitId = await prisma.$transaction(async (tx) => {
      if (duzenleme) {
        ...
        if (!onceki || onceki.silindi || onceki.tip !== "VERILEN") throw new BulunamadiHatasi()
      ...
      // Dikkat: "SIPARIS" türü alınan siparişin sayacı — burada AYRI bir
      // tür (SIPARIS_VERILEN) kullanılıyor, yoksa iki yön aynı sayacı/
      // ön eki paylaşır ve numaralar (SP.../SV...) birbirine karışırdı.
      const siparisNo = await siradakiNumara(tx, "SIPARIS_VERILEN", {
        yilBazli: true,
        varsayilanOnEk: `SV${new Date().getFullYear()}-`,
        basamak: 5,
      })
  ...
  revalidatePath(`/siparis/verilen/${kayitId}`)
  redirect(`/siparis/verilen/${kayitId}?kaydedildi=1`)
}
...
export async function kalemKaydet(
  ...
  revalidatePath(`/siparis/verilen/${v.siparisId}`)
  return {}
}
...
export async function kalemSil(kalemId: number): Promise<KalemDurumu> {
  ...
  revalidatePath(`/siparis/verilen/${siparisId}`)
  return {}
}
 ...
 * evrak modülünün aksine STOK ARTMAZ, TEDARİKÇİYE BORÇ YAZILMAZ — bunlar
 * ancak siparişten faturaya dönüştürüldüğünde (adım 9.7) doğar. Onay yalnız
 * "bu sipariş kesinleşti, sevkiyat/teslim alım takibine hazır" anlamına gelir.
 */
...
/**
 * Seçilen satırları/miktarları alış faturasına (TASLAK) dönüştürür — ortak
 * mantık `siparis/donustur.ts`'te, alınan tarafının (`siparis/alinan/actions.ts`)
 * aynası, tek fark `beklenenTip`/`evrakTur` ve hedef ekran.
 */
export async function verilenFaturayaDonustur(
  ...
  redirect(`/evrak/alis/${evrakId}?donusturuldu=1`)
}

// ============================================================================

/** Kalem satırındaki stok arama kutusunu besler — alış fiyatını öneren varyant. */
