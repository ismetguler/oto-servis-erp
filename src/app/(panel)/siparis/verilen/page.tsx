...
import { verilenSiparisleriGetir } from "./veri"
import { SiparisDurumRozeti } from "@/components/siparis/durum-rozeti"
import { Button } from "@/components/ui/button"
import { para, tarih } from "@/lib/bicim"
import { yetkiliOturum } from "@/lib/oturum"

export const metadata: Metadata = { title: "Verilen Siparişler" }
export const dynamic = "force-dynamic"

type Aramalar = { q?: string; durum?: string; bas?: string; bit?: string }

/** Verilen sipariş listesi — Alınan Siparişler listesindeki desenin aynısı. */
export default async function VerilenSiparisler({
  searchParams,
}: {
  ...
  const siparisler = await verilenSiparisleriGetir(f)
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[1rem] font-semibold">Verilen Siparişler</h1>
          <p className="text-[0.8125rem] text-muted-foreground">
            {siparisler.length} sipariş · geçerli toplam {para(genelToplam)}
          </p>
        </div>
        <Button size="sm" asChild>
          <Link href="/siparis/verilen/yeni">
