...
import { alinanFaturayaDonustur } from "../../actions"
...
export default async function AlinanFaturala({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="flex flex-col gap-4 p-4">
      <div>
        ...
        <p className="text-[0.8125rem] text-muted-foreground">
          {siparis.cari.unvan} ({siparis.cari.kod}) · her satır için sevk edilecek miktarı girin, kalanı
          aşamaz. Kısmi sevkiyat desteklenir.
        </p>
      </div>

      {satirlar.length === 0 ? (
        <p className="panel px-3 py-2 text-[0.8125rem] text-muted-foreground">
          Bu siparişin tüm satırları zaten tamamen sevk edilmiş.
        </p>
      ) : (
        <FaturayaDonusturFormu
          siparisId={siparis.id}
          satirlar={satirlar}
          aksiyon={alinanFaturayaDonustur}
          hedefEtiket="Satış Faturası"
          geriYolu={`/siparis/alinan/${siparis.id}`}
        />
      )}
    </div>
  )
