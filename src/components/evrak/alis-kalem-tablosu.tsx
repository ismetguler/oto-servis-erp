...
import {
  ...
  katalogAraAction,
  type KalemDurumu,
} from "@/app/(panel)/evrak/alis/actions"
...
/**
 * ALIŞ FATURASI KALEMLERİ — satış faturasındaki `KalemTablosu`nun aynası.
 * Fark: stok arama kutusu (`katalogAraAction`) burada `alisFiyat`ı öneriyor,
 * satış fiyatını değil (`evrak/alis/veri.ts` → `katalogAraAlis`).
 */
...
export function AlisKalemTablosu({
  evrakId,
  kalemler,
  duzenlenebilir,
}: {
...
/** Stok arama kutusu — satıştaki `KatalogArama` ile aynı davranış, alış fiyatı önerir. */
