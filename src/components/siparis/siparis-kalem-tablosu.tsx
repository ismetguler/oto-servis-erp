...
import {
  ...
  katalogAraAction,
  type KalemDurumu,
} from "@/app/(panel)/siparis/alinan/actions"
...
/**
 * ALINAN SİPARİŞ KALEMLERİ — `AlisKalemTablosu`nun aynası, tek fark: bir
 * "Bakiye" sütunu var — `miktar - sevkMiktar` (ne kadarı henüz sevk
 * edilmedi). Adım 9.7'ye kadar `sevkMiktar` hep 0 olduğundan bakiye = miktar
 * görünür; dönüştürme akışı açılınca bu sütun gerçek değeri gösterecek.
 */
...
export function SiparisKalemTablosu({
  siparisId,
  kalemler,
  duzenlenebilir,
}: {
  ...
  return (
    <div className="rounded-md border border-border bg-card">
      ...
      {formAcik ? (
        <KalemFormu key={duzenlenen?.id ?? "yeni"} siparisId={siparisId} kalem={duzenlenen} kapat={formuKapat} />
      ) : null}
        ...
        <table className="w-full text-[0.8125rem]">
          <thead>
            <tr className="border-b border-border text-[0.75rem] text-muted-foreground">
              ...
              <th className="px-2 py-1.5 text-right font-medium">Sevk</th>
          ...
          <tbody>
            {kalemler.length === 0 ? (
              ...
              kalemler.map((k, sira) => {
                const h = kalemHesapla(k)
                const bakiye = Math.max(0, k.miktar - k.sevkMiktar)
                return (
                  <tr key={k.id} className="border-b border-border/60 last:border-0">
                    ...
                    <td className="px-2 py-1.5 text-right tabular-nums text-muted-foreground">
                      {miktarBicim(k.sevkMiktar)}
                    </td>
                    <td className="px-2 py-1.5 text-right tabular-nums">
                      <span className={bakiye > 0 ? "font-medium text-uyari" : "text-basari"}>
                        {miktarBicim(bakiye)}
                      </span>
                    </td>
...
/** Stok arama kutusu — evrak modülündeki `KatalogArama` ile aynı davranış. */
